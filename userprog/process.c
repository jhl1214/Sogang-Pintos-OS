...
int parse_filename(char *file_name, char **argv){
	int count = 0;
	char *save_ptr;

	for(argv[count] = strtok_r(file_name, " ", &save_ptr);argv[count] != NULL;argv[count] = strtok_r(NULL, " ", &save_ptr))
			count++;

	return count;
}

void construct_ESP(void **esp, char **argv, int argc){
	int i, arg_len, align;
	int total_len = 0;
	int *arg_address[128];
	...
	align = 4 - (total_len % 4);
	*esp = *esp - align;
	memset(*esp, (uint8_t)0, align);

	// argv[count]
	*esp = *esp - sizeof(char *);
	memset(*esp, (uint8_t)0, sizeof(char *));
	...
	// argv
	*esp = *esp - sizeof(char *);
	memcpy(*esp, &arg_address[0], sizeof(char **));
	...
	*esp = *esp - sizeof(int);
	...
	// return
	*esp = *esp - sizeof(void*);
	memset(*esp, (uint8_t)0, sizeof(void *));
}
